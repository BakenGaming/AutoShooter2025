...
[CreateAssetMenu(menuName ="Weapons/Quad Weapon")]
public class QuadShot : Weapon
{
    ...
    public override void InitializeWeapon(WeaponSystem _w, StaticWeaponStats _s)
    {
        ...
        GameManager.i.SetupObjectPools(stats.spawnedObject.GetComponent<QuadProjectileHandler>(), 50, "Quad", PoolType.Projectiles);
    }
    ...
    public override void TryActivateWeapon()
    {
        if(readyToFire)
        {
            for(int i = 0; i < 4; i++)
            {
            QuadProjectileHandler newProjectile = ObjectPooler.DequeueObject<QuadProjectileHandler>("Quad", PoolType.Projectiles);
